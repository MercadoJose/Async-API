...
public class AsyncService {
	...
	@Async("asyncExecutor")
	public CompletableFuture<EmployeeProfile> getEmployeeProfiles() throws InterruptedException {
		log.info("getEmployeeProfiles Starts");
		EmployeeProfile employeeProfile = restTemplate.getForObject("http://localhost:8082/profiles",
				EmployeeProfile.class);

		log.info("employeeProfiles, {}", employeeProfile);
		Thread.sleep(1000L); // Intentional delay
		log.info("employeeProfiles completed");
		return CompletableFuture.completedFuture(employeeProfile);
	}
